...
public class CollaboratorServiceImpl extends RemoteServiceServlet implements
	...
	public LockedDocument lockDocument(String documentKey)
		...
		if ((lockedTime==null) || lockedTime.before(new Date())){
			...
			return newLock;						
				...
				throw new LockUnavailable("Document currently locked until "+lockedTime.toString());
			}
	...
	/* Helper function for use outside this file */
	public boolean isValid(LockedDocument lDoc) throws LockExpired{
		return isValid(lDoc.getLockedUntil());
	}
	...
	@SuppressWarnings("deprecation") // Because the operations to set the time on the Date class are deprecated
	/* This function is used to renew the lock of an active document */
	public LockedDocument renewLock(LockedDocument lDoc) throws LockExpired{
		LockedDocument locked = lDoc;
		String key = locked.getKey();
		Transaction txn = datastore.beginTransaction();
		try{
			Date currentTime = new Date();
			if (isKeyPresent(key)){
				// Getting here means the key is present in the MetadataList
				Entity document = datastore.get(KeyFactory.stringToKey(key));
				if (document.getProperty("LockedBy").equals(locked.getLockedBy()) &&
						currentTime.before(locked.getLockedUntil())){
					// getting here means the ID matched and the lock is current
					currentTime.setMinutes(currentTime.getMinutes()+10);
					lDoc.setLockedUntil(currentTime);
					document.setProperty("Date", currentTime);
					datastore.put(txn, document);
					txn.commit();
					return locked;
				}else{
			...
			// This should never occur, because we check whether the key is there before referencing the datastore
		}finally{
			if(txn.isActive()){
				txn.rollback();
			}
